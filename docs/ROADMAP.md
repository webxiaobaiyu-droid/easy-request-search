...
- [x] 3. 失败 / 取消请求的完整状态
  - 状态分组增加「失败」（status < 0，如 net::ERR_FAILED、CORS 失败、取消）。
  - 列表状态列显示「失败」，详情头部展示失败原因（statusText）。
- [x] 4. 复制为 fetch / axios
...
- 2026-08-22　完成第 6–7 项（第二梯队全部）。测试 59 个全部通过，vue-tsc 无错误。
  - 6. 虚拟滚动 + 缓冲上限：`applyEventBatch` 支持 `maxEntries` 裁剪（面板保留最近 5000 条，快照同样裁剪）；RequestList 改为固定行高（21px）窗口化渲染（上下 padding 占位 + overscan 12 行），4800 条请求只渲染约 48 行。
